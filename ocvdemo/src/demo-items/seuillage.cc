...
Seuillage::Seuillage() { props.id = "seuillage"; }
...
int Seuillage::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  // Otsu
    ...
    cv::threshold(Ig, output.images[0], 0 /* non utilisé */, 255,
                  cv::THRESH_BINARY_INV | cv::THRESH_OTSU);
  }
  // Adaptatif
  else if (sel == 2) {
    int taille_bloc =
        input.model.get_attribute_as_int("seuillage-adaptatif/taille-bloc");
...
DTransDemo::DTransDemo() { props.id = "dtrans"; }
...
int DTransDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  cv::threshold(Ig, output.images[0], 0 /* non utilisé */, 255,
                cv::THRESH_BINARY_INV | cv::THRESH_OTSU);
  infos("dtrans...");
  cv::distanceTransform(output.images[0], tmp, cv::DIST_L2, 3);
  infos("ok.");
  cv::normalize(tmp, output.images[0], 0, 255, cv::NORM_MINMAX, CV_8UC1);
  return 0;
}
