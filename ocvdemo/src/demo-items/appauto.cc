...
cv::Ptr<cv::ml::StatModel> creation_classifieur_svm(utils::model::Node &model) {
  ...
  svm->setGamma(gamma);
  svm->setKernel((cv::ml::SVM::KernelTypes)kernel); // SVM::RBF);
  svm->setNu(0.5);
  svm->setP(0.1);
  svm->setC(C);
  svm->setType(cv::ml::SVM::C_SVC); // ou NU_SVC
  svm->setDegree(degre);
  return svm;
}

cv::Ptr<cv::ml::StatModel> creation_classifieur_knn(utils::model::Node &model) {
...
creation_classifieur_adaboost(utils::model::Node &model) {
  // A FAIRE : créer un classifieur Adaboost
  cv::Ptr<cv::ml::Boost> res = cv::ml::Boost::create();
...
int DemoAppAuto::proceed(OCVDemoItemInput &entree, OCVDemoItemOutput &sortie) {
  ...
  uint32_t n = entree.model.get_attribute_as_int("napp"), ntraits = 2u;
...
#define MAX_CLASSES 4
  ...
  for (auto i = 0u; i < n; i++) {
    ...
    cv::line(O, cv::Point(xi - dl, yi), cv::Point(xi + dl, yi),
             couleurs[classe], 1, cv::LINE_AA);
    cv::line(O, cv::Point(xi, yi - dl), cv::Point(xi, yi + dl),
             couleurs[classe], 1, cv::LINE_AA);
  ...
  svm->setTermCriteria(TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 1000,
  1e-3)); svm->setGamma(gamma); svm->setKernel((SVM::KernelTypes) kernel);
  //SVM::RBF); svm->setNu(0.5); svm->setP(0.1); svm->setC(C);//0.01);
  svm->setType(SVM::C_SVC);
  svm->setDegree(degre);*/

  cv::Ptr<cv::ml::StatModel> smodel;
  ...
  smodel->train(mat_entrainement, cv::ml::ROW_SAMPLE, labels);
  trace_verbeuse("Ok.");
