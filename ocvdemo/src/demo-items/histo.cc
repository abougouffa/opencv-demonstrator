...
static void calcul_histogramme_1d(const cv::Mat &I, cv::MatND &hist, int canal,
                                  int maxval, int nbins) {
  float hranges[] = {0, (float)maxval};
  const float *ranges[] = {hranges};

  cv::calcHist(&I, 1, &canal, cv::Mat(), // (pas de masque)
               hist, 1, &nbins, ranges,
...
static void calcul_histogramme_2d(const cv::Mat &I, cv::MatND &hist,
                                  int canaux[], int maxval[], int nbins[]) {
  float hranges0[] = {0, (float)maxval[0]};
  float hranges1[] = {0, (float)maxval[1]};
  const float *ranges[] = {hranges0, hranges1};

  cv::calcHist(&I, 1, canaux, cv::Mat(), // (pas de masque)
               hist, 2, nbins, ranges,
               true,   // Histogram uniforme
...
HistoCalc::HistoCalc() { props.id = "hist-calc"; }

static void dessine_courbe(const cv::MatND &x, cv::Mat &image, cv::Scalar color,
                           float yscale) {
  uint32_t n = x.rows;
  ...
  for (auto i = 1u; i < n; i++) {
    ...
    line(image, cv::Point((i - 1) * sx / n, sy - lxi),
         cv::Point(i * sx / n, sy - xi), color, 2, cv::LINE_AA);
...
int HistoCalc::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  if (sel == 0) {
    ...
    output.images[0] =
        cv::Mat(cv::Size(512, 512), CV_8UC3, cv::Scalar(255, 255, 255));

    for (auto i = 0u; i < 3; i++)
      calcul_histogramme_1d(I, hist[i], i, 255, nbins);

    dessine_courbe(hist[0], output.images[0], cv::Scalar(255, 0, 0),
                   512.0 / 255);
    dessine_courbe(hist[1], output.images[0], cv::Scalar(0, 255, 0),
                   512.0 / 255);
    dessine_courbe(hist[2], output.images[0], cv::Scalar(0, 0, 255),
                   512.0 / 255);
    ...
    for (auto i = 0u; i < 3; i++) {
      ...
      output.images[i] =
          cv::Mat(cv::Size(512, 512), CV_8UC3, cv::Scalar(255, 255, 255));
      dessine_courbe(hist, output.images[i], cv::Scalar(0, 0, 0),
                     512.0 / 255.0);
    }
...
HistoEgalisationDemo::HistoEgalisationDemo() { props.id = "histeq"; }

int HistoEgalisationDemo::proceed(OCVDemoItemInput &input,
