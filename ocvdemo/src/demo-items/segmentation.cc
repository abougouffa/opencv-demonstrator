...
DemoSuperpixels::DemoSuperpixels() { props.id = "superpixels"; }

int DemoSuperpixels::proceed(OCVDemoItemInput &input,
...
int DemoMahalanobis::proceed(OCVDemoItemInput &input,
                             OCVDemoItemOutput &output) {
...
int WShedDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  cv::Mat gray, nb, ret;
  cv::cvtColor(input.images[0], gray, cv::COLOR_BGR2GRAY);
  cv::threshold(gray, nb, 0, 255, cv::THRESH_BINARY_INV | cv::THRESH_OTSU);

  // Execute morphological-open
  morphologyEx(nb, ret, cv::MORPH_OPEN, cv::Mat::ones(9, 9, CV_8SC1),
               cv::Point(4, 4), 2);
  cv::Mat distTransformed(ret.rows, ret.cols, CV_32F);
  cv::distanceTransform(ret, distTransformed, cv::DIST_L2, 3);
  // normalize the transformed image in order to display
  cv::normalize(distTransformed, distTransformed, 0.0, 255.0, cv::NORM_MINMAX);
  ...
  cv::threshold(distTransformed, distTransformed,
                input.model.get_attribute_as_float("seuil-dist") * 255, 255,
                cv::THRESH_BINARY);
  ...
  cv::findContours(distTransformed, contours, hierarchy, cv::RETR_CCOMP,
  ...
  cv::watershed(input.images[0], markers);
