...
int ContourDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  Mat K = getStructuringElement(kernel_type,
                                      Size(2*kernel_width + 1,
  2*kernel_width+1), Point(kernel_width, kernel_width));
  ...
  findContours(masque_canny, contours, hierarchie, mode,
  ...
  for (auto i = 0u; i < contours.size(); i++) {
    cv::Scalar couleur = cv::Scalar(rng.uniform(0, 255), rng.uniform(0, 255),
                                    rng.uniform(0, 255));
    cv::drawContours(dessin, contours, i, couleur, 2, 8, hierarchie, 0,
                     cv::Point());
  }
...
GradientDemo::GradientDemo() { props.id = "gradient"; }
...
int GradientDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  if (sigma > 0) {
    if (sel3 == 0)
      cv::GaussianBlur(I, tmp, cv::Size(0, 0), sigma);
    else if (sel3 == 1)
  ...
  if (sel2 == 0) {
    cv::Sobel(tmp, gx, CV_32F, 1, 0, tnoyau, 1, 0);
    cv::Sobel(tmp, gy, CV_32F, 0, 1, tnoyau, 1, 0);
  } else {
    cv::Scharr(tmp, gx, CV_32F, 1, 0);
    cv::Scharr(tmp, gy, CV_32F, 0, 1);
  }
  ...
  if (!preconv && (!gradient_couleur || (sel == 2))) {
    ...
    gx = gx / 255;
    gy = gy / 255;
    cv::cvtColor(gx, gx, cv::COLOR_BGR2GRAY);
    cv::cvtColor(gy, gy, cv::COLOR_BGR2GRAY);
  }
  ...
  if (sel == 0) {
    ...
    /*if(mode_hsv)
    {
      infos("mag : %d, %d, masque : %d, %d", mag.cols, mag.rows,
    masque_saturation_luminance.cols, masque_saturation_luminance.rows);
...
DetFlouDemo::DetFlouDemo() { props.id = "det-flou"; }
...
LaplaceDemo::LaplaceDemo() { props.id = "laplace"; }
...
int LaplaceDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  if (pref && (sigma > 0))
    cv::GaussianBlur(tmp, tmp, cv::Size(0, 0), sigma);
...
CannyDemo::CannyDemo() { props.id = "canny"; }
...
HoughDemo::HoughDemo() { props.id = "hough"; }
...
int HoughDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  float reso_theta =
      input.model.get_attribute_as_float("reso-theta") * CV_PI / 180.0;
  ...
  cv::cvtColor(bw, output.images[0], cv::COLOR_GRAY2BGR);
  ...
  if (sel == 0) {
    ...
    std::vector<cv::Vec2f> lignes;
    cv::Mat deb;
    ocvext::Hough_lines_with_gradient_dir(I, lignes, deb, reso_rho, reso_theta,
                                          0.6, seuilg);
    cv::pyrUp(deb, deb);
...
HoughCDemo::HoughCDemo() { props.id = "houghc"; }
...
int HoughCDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  cv::HoughCircles(gris, cercles, cv::HOUGH_GRADIENT, 2 /* dp = 2 */,
                   20 /* min dist */, seuil_canny, seuil, rmin, rmax);
  infos("Détecté %d cercles.\n", (int)cercles.size());
  for (size_t i = 0; i < cercles.size(); i++) {
    float xc = cercles[i][0], yc = cercles[i][1], r = cercles[i][2];
    cv::circle(output.images[0], cv::Point(xc, yc), r, cv::Scalar(0, 255, 0),
               2);
  }
...
RectDemo::RectDemo() { props.id = "hough-rec"; }
...
int RectDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  for (auto i = 0u; i < lines.size(); i++) {
    cv::Vec4i v = lines[i];
    cv::line(output.images[0], cv::Point(v[0], v[1]), cv::Point(v[2], v[3]),
             CV_RGB(0, 255, 0));
  }
