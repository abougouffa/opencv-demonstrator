...
int IFTDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  infos("plan0 = %d,%d, plan1 = %d,%d", plans[0].cols, plans[0].rows,
        plans[1].cols, plans[1].rows);
...
int DFTDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  if (source == 1) {
    ...
    for (auto y = 0u; y < 512; y++) {
      ...
      for (auto x = 0u; x < 512; x++) {
        float xf = x;
        //*ptr++ = 128.0 + 128 * std::sin(2 * CV_PI * xf / (512 * (periode /
        // 512.0)));
  ...
  copyMakeBorder(Ig, padded, 0, m - Ig.rows, 0, n - Ig.cols, BORDER_CONSTANT,
  Scalar::all(0));

  Mat planes[] = {Mat_<float>(padded), Mat::zeros(padded.size(), CV_32F)};

  merge(planes, 2, complexI);         // Add to the expanded another plane with
  zeros
  */
...
int DemoDetectionTranslation::proceed(OCVDemoItemInput &input,
...
int DemoDetectionRotation::proceed(OCVDemoItemInput &input,
                                   OCVDemoItemOutput &output) {
  cv::Mat I, mag, O;
  cv::cvtColor(input.images[0], I, cv::COLOR_BGR2GRAY);
  I.convertTo(I, CV_32F);
  ...
  if (mode_log)
    cv::logPolar(mag, O, cv::Point(mag.cols / 2, mag.rows / 2), mag.cols / 2,
                 cv::INTER_CUBIC);
  else
    cv::linearPolar(mag, O, cv::Point(mag.cols / 2, mag.rows / 2), mag.cols / 2,
                    cv::INTER_CUBIC);
...
DemoSousSpectrale::DemoSousSpectrale() { props.id = "sous-spect"; }

static void sousstraction_spectrale_gs(cv::Mat &I, cv::Mat &O, cv::Mat &mag,
                                       cv::Mat &masque, float seuil) {
  cv::Mat F, plans[2];
  cv::dft(I, F, cv::DFT_COMPLEX_OUTPUT, I.rows);

  // infos("F: cols=%d, rows=%d, type=%d, nchn=%d", F.cols, F.rows, F.type(),
  // F.channels());
  ...
  cv::dft(F, O, cv::DFT_INVERSE + cv::DFT_SCALE + cv::DFT_REAL_OUTPUT,
          I.rows); // + K_spatial[i].rows / 2);

  cv::normalize(O, O, 0, 255, cv::NORM_MINMAX);
}

static void sousstraction_spectrale(cv::Mat &I, cv::Mat &O, cv::Mat &mag,
                                    float seuil) {
...
int DemoSousSpectrale::proceed(OCVDemoItemInput &input,
                               OCVDemoItemOutput &output) {
  cv::Mat mag, O, I = input.images[0], If;
  I.convertTo(If, CV_32F);

  sousstraction_spectrale(If, O, mag,
                          input.model.get_attribute_as_float("seuil"));
...
DemoDetectionPeriode::DemoDetectionPeriode() { props.id = "detection-periode"; }

int DemoDetectionPeriode::proceed(OCVDemoItemInput &input,
                                  OCVDemoItemOutput &output) {
  ...
  infos("Ig: %d,%d, r: %d,%d,%d,%d", Ig.cols, Ig.rows, r.x, r.y, r.width,
        r.height);
