...
int MatchDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  // if(!lock)
  {
    ...
    if (sel == 0)
      detecteur =
          cv::ORB::create(input.model.get_attribute_as_int("npts")); // OCV 3.0
    else
    ...
    if (good_matches.size() > 0)
      cv::drawMatches(imgs[0], kpts[0], imgs[1], kpts[1], good_matches,
                      output.images[0]);

    infos("draw match ok: %d assoc, %d ok.", matches.size(),
          good_matches.size());
...
ScoreShiTomasi::ScoreShiTomasi() { props.id = "score-harris"; }

int ScoreShiTomasi::proceed(OCVDemoItemInput &input,
...
CornerDemo::CornerDemo() { props.id = "corner-det"; }
...
int CornerDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  cv::cvtColor(input.images[0], gris, cv::COLOR_BGR2GRAY);
  ...
  output.images[0] = input.images[0].clone();
  infos("drawK");
  cv::drawKeypoints(input.images[0], keypoints, output.images[0],
                    cv::Scalar(0, 0, 255));
  infos("ok");
  return 0;
}
...
int VisageDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  cv::cvtColor(I, frame_gray, cv::COLOR_BGR2GRAY);
  ...
  for (size_t i = 0; i < faces.size(); i++) {
    ...
    cv::rectangle(
        output.images[0], cv::Point(faces[i].x, faces[i].y),
...
CascGenDemo::CascGenDemo(std::string id) : rng(12345) {
  ...
  // I do not know if this is correct but setting nout to 1 seems to do the
  // right thing.
  // output.nout = 0;
  output.nout = 1;
...
int CascGenDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  //-- Detect faces
  cascade[sel].detectMultiScale(frame_gray, rdi, 1.1, /* facteur d'échelle */
                                2,                    /* min voisins ? */
  ...
  for (size_t i = 0; i < rdi.size(); i++) {
    cv::Point center(rdi[i].x + rdi[i].width * 0.5,
                     rdi[i].y + rdi[i].height * 0.5);
    cv::rectangle(output.images[0], rdi[i], cv::Scalar(0, 255, 0), 3);
  }
  return 0;
}

DemoHog::DemoHog() { props.id = "hog"; }
...
int DemoHog::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  cv::Mat img;
  cv::cvtColor(input.images[0], img, cv::COLOR_BGR2GRAY);
  ...
  hog.derivAperture = 0;
  hog.winSigma = -1;
  hog.histogramNormType = cv::HOGDescriptor::L2Hys;
...
int DemoFaceRecognizer::proceed(OCVDemoItemInput &input,
                                OCVDemoItemOutput &output) {
#ifdef USE_CONTRIB
  uint16_t sx = 0, sy = 0;
  ...
  if (algo <= 2) {
    ...
    if (algo == 0) {
      ...
      model =
          cv::face::createLBPHFaceRecognizer(); // radius, neighbors, gx, gy);
    }
    ...
    infos("Ntests = %d, nfaux = %d, taux de réussite = %.2f %%.", ntests, nfaux,
          taux * 100);

    output.nout = 0;

    if (algo <= 1) {
      cv::face::BasicFaceRecognizer *model2 =
          (cv::face::BasicFaceRecognizer *)model.get();
