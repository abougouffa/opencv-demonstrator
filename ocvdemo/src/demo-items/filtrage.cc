...
DemoFiltreGabor::DemoFiltreGabor() { props.id = "gabor"; }

int DemoFiltreGabor::proceed(OCVDemoItemInput &input,
  ...
  cv::Mat K = cv::getGaborKernel(cv::Size(taille_noyau, taille_noyau), sigma,
                                 theta, lambda, gamma, psi);
...
DemoRedim::DemoRedim() { props.id = "redim"; }
...
int DemoFiltrage::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  config.gaussien.taille_noyau =
      model.get_attribute_as_int("gaussian/taille-noyau");
  ...
  config.median.taille_noyau =
      model.get_attribute_as_int("median/taille-noyau");
  if ((config.median.taille_noyau % 2) == 0) {
    config.median.taille_noyau++;
    model.set_attribute("median/taille-noyau", config.median.taille_noyau);
  }
  config.bilateral.taille_noyau =
      model.get_attribute_as_int("bilateral/taille-noyau");
  config.bilateral.sigma_couleur =
      model.get_attribute_as_float("bilateral/sigma-color");
  config.bilateral.sigma_espace =
      model.get_attribute_as_float("bilateral/sigma-space");
  return proceed(config, input.images[0], output);
}

int DemoFiltrage::proceed(const DemoFiltrageConfig &conf, cv::Mat &I,
                          OCVDemoItemOutput &output) {
  cv::Mat Ib, If;
  ...
  if (conf.type_filtre == DemoFiltrageConfig::FILTRE_MA)
    ...
    cv::GaussianBlur(
        Ib, If,
        cv::Size(conf.gaussien.taille_noyau, conf.gaussien.taille_noyau),
        conf.gaussien.sigma);
    ...
    cv::bilateralFilter(Ib, If, conf.bilateral.taille_noyau,
                        conf.bilateral.sigma_couleur,
                        conf.bilateral.sigma_espace);
