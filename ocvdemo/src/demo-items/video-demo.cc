...
DemoVideoStab::DemoVideoStab() { props.id = "videostab"; }
...
void CamShiftDemo::set_roi(const cv::Mat &I, const cv::Rect &new_roi) {
  ...
  trace_majeure("set roi(%d,%d,%d,%d): calc bp...", input.roi.x, input.roi.y,
                input.roi.width, input.roi.height);
...
int CamShiftDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  if (bp_init_ok) {
    ...
    if (trackwindow.width * trackwindow.height > 0) {
      ...
      trace_verbeuse("trackwindow: %d, %d, %d, %d.", trackwindow.x,
                     trackwindow.y, trackwindow.width, trackwindow.height);
    } else
...
int SousArrierePlanDemo::proceed(OCVDemoItemInput &input,
  ...
  output.images[0] = I;
  cv::resize(I, tmp, cv::Size(0, 0), 0.25, 0.25);
  algo->apply(tmp, mask);
  ...
  cv::updateMotionHistory(mask, mhi, nframes - 5, maxhist);
  ...
  infos("segmentMotion...");
  cv::segmentMotion(mhi, segmask, brects, nframes - 5, seuil);

  infos("rects...");
  images[1] = tmp.clone();
  for (auto r : brects)
    cv::rectangle(images[1], r, cv::Scalar(0, 255, 0));
  cv::resize(images[1], images[1], cv::Size(0, 0), 4, 4);
...
int OptFlowDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output) {
  ...
  cv::normalize(mag, nmag, 0, 1.0, cv::NORM_MINMAX);
  ...
  cv::cvtColor(hsv, output.images[1], cv::COLOR_HSV2BGR);
  output.images[1].convertTo(output.images[1], CV_8UC3, 255);
  trace_verbeuse("fait: %d * %d, depth = %d.", output.images[0].cols,
                 output.images[0].rows, output.images[0].depth());
  return 0;
}
