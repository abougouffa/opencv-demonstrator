...
void ImageMosaique::update_image(int index, const cv::Mat &img) {
  ...
  mutex.lock();
  cv::Rect rect(img_pos[index].x, img_pos[index].y, img_pos[index].width,
                img_pos[index].height);
  cv::Mat roi(disp_img, rect);

  cv::Mat im = img.clone();

  while ((im.cols >= 2 * roi.size().width) ||
         (im.rows >= 2 * roi.size().height))
    cv::pyrDown(im, im);
...
int ImageMosaique::show_multiple_images(std::string title,
  ...
  infos("show_multiple_images (sizex = %d, sizey = %d, w = %d, h = %d, W = %d, "
        "H = %d)",
        sizex, sizey, w, h, W, H);
  ...
  for (i = 0, m = 20, n = 20; i < (int)nimages; i++, m += (20 + sizex)) {
    ...
    // trace_verbeuse("di: %d*%d, r:%d,%d,%d,%d.", disp_img.cols, disp_img.rows,
    ...
    // Resize the input image and copy the it to the Single Big Image
    {
      ...
      while ((im.cols >= 2 * roi.size().width) ||
             (im.rows >= 2 * roi.size().height))
        cv::pyrDown(im, im);
      while ((im.cols <= roi.size().width / 2) &&
             (im.rows <= roi.size().height / 2))
        cv::pyrUp(im, im);
    ...
    if (show_deco && (titles[i].size() > 0)) {
      ...
      /// int dx = (x/scale);
      int xc = m + (x / (2 * scale));
      cv::putText(disp_img, texte,
                  cv::Point(xc - si.width / 2, n + y / scale + 1.5 * si.height),
                  font, tscale, cv::Scalar(255, 255, 255), 1.2, cv::LINE_AA);
    }
  }
  ...
  if (model.get_attribute_as_boolean("force-taille-sortie")) {
    ...
    cv::resizeWindow(title.c_str(), model.get_attribute_as_int("sortie-sx"),
  ...
  trace_verbeuse("imshow: [%s], %d * %d", title.c_str(), disp_img.cols,
                 disp_img.rows);
  cv::imshow(title.c_str(), disp_img);
