...
void ImageSelecteur::maj_selection() {
  ...
  pixbuf = Gdk::Pixbuf::create_from_data(
      bigmat.data, Gdk::Colorspace::COLORSPACE_RGB, false, 8, bigmat.cols,
      bigmat.rows, 3 * bigmat.cols);
...
void ImageSelecteur::maj_mosaique() {
  ...
  if (!OCVDemo::get_instance()->get_modele_global().get_attribute_as_boolean(
          "afficher-sources"))
    return;
  ...
  for (auto i = 0u; i < n; i++) {
    ...
    trace_verbeuse("resize(%d,%d,%d,%d,%d,%d)", im.px, im.py, col_width,
                   row_height, col_width, row_height);
    ...
    if (ratio_aspect_orig > ratio_aspect_sortie) {
      ...
      cv::resize(tmp, bigmat(cv::Rect(im.px, py, img_width, hauteur)),
    ...
    // Doit ajouter du padding horizontal
      ...
      cv::resize(tmp, bigmat(cv::Rect(px, im.py, largeur, img_height)),
...
void ImageSelecteur::on_size_change(Gtk::Allocation &alloc) { maj_mosaique(); }
...
ImageSelecteur::ImageSelecteur() {
  ...
  evt_box.signal_button_press_event().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_b_pressed));
  evt_box.signal_button_release_event().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_b_released));

  evt_box.signal_key_release_event().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_k_released));

  gtk_image.signal_size_allocate().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_size_change));

  b_open.signal_clicked().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_b_open));
  b_ajout.signal_clicked().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_b_add));
  b_suppr.signal_clicked().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_b_del));
  b_suppr_tout.signal_clicked().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_b_del_tout));
  b_maj.signal_clicked().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_b_maj));
  ...
  signal_drag_data_received().connect(
      sigc::mem_fun(*this, &ImageSelecteur::on_dropped_file));
}

void ImageSelecteur::on_dropped_file(
...
bool ImageSelecteur::has_video() { return has_a_video; }
...
unsigned int ImageSelecteur::get_nb_images() const { return images.size(); }
  ...
  if ((ext == "mpg") || (ext == "avi") || (ext == "mp4") || (ext == "wmv")) {
    ...
    if (!vc.isOpened()) {
      utils::mmi::dialogs::affiche_erreur(
          "Error", "Error while loading video",
    ...
    if (!vc.isOpened()) {
      utils::mmi::dialogs::affiche_erreur(
          "Error", "Error while connecting to webcam",
          "Maybe the webcam is not supported or is already used in another "
          "application.");
      return;
    }
...
bool ImageSelecteur::on_b_pressed(GdkEventButton *event) {
  ...
  for (auto i = 0u; i < images.size(); i++) {
    auto &img = images[i];
    if ((x > img.px) && (y > img.py) && (x < img.px + this->img_width) &&
