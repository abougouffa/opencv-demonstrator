  ...
  for (;;) {
    ...
    case ODEvent::CALCUL:
      try {
        ...
        //				  langue.get_item("err-opencv-msg"),
        // langue.get_item("err-opencv-msg2"));
        break;
      }
...
void OCVDemo::thread_video() {
  ...
  debut:
    ...
    while (!entree_video || !are_all_video_ok() ||
           (video_captures.size() == 0)) {
      video_en_cours = false;
      signal_video_suspendue.raise();
      signal_video_demarre.wait();
    }
...
void OCVDemo::update() {
  ...
  infos("Calcul [%s], img: %d*%d, %d chn, model =\n%s",
        demo_en_cours->props.id.c_str(), I0.cols, I0.rows, I0.channels(),
        s.c_str());
  ...
  if (calcul_status) {
    ...
    if (langue.has_item(s))
      s = langue.get_item(s);
    utils::mmi::dialogs::affiche_avertissement(
        "Erreur de traitement", langue.get_item("echec-calcul"), s);
    mutex_update.unlock();
    return;
  } else {
  ...
  if (img_count &&
      (demo_en_cours->output.images[img_count - 1].data == nullptr)) {
    avertissement("Img count = %d, et image de sortie non initialisée.",
                  img_count);
    img_count = 1;
  } else if (img_count == 0) {
    if (Ia.data != nullptr)
  ...
  for (auto j = 0u; j < img_count; j++) {
    ...
    //  titres.push_back(utils::str::utf8_to_latin(s)); // à passer en utf-8 dès
    //  que fenêtre GTK fait
    titres.push_back(s);
  }
...
void OCVDemo::on_event(const ChangeEvent &ce) {
  trace_verbeuse("change-event: %d / %s", (int)ce.type,
                 ce.path[0].name.c_str());
...
void OCVDemo::on_event(const OCVDemoItemRefresh &e) { update(); }
    ...
    if (se.is_video()) {
      ...
      if (!res) {
        utils::mmi::dialogs::affiche_erreur(
            langue.get_item("ech-vid-tit"), langue.get_item("ech-vid-sd"),
...
void OCVDemo::setup_demo(const utils::model::Node &sel) {
  ...
  cv::namedWindow(titre_principal, cv::WINDOW_NORMAL);
  ...
  for (auto demo : items) {
    if (demo->props.id == id) {
      ...
      */
      ...
      // Réinitialisation des images de sorties (au cas où elles pointaient vers
      // les images d'entrées) Resetting outputs images ( in case they pointed
      // to the pictures of inputs )
      // for(auto i = 0u; i < DEMO_MAX_IMG_OUT; i++)
      //  demo->
      ...
      */
      ...
      if (nmissing > 0) {
        for (auto i = 0; i < nmissing; i++)
          img_selecteur.ajoute_fichier(utils::get_fixed_data_path() +
                                       "/img/lena.jpg");
      }
...
void OCVDemo::compute_Ia() {
  if ((demo_en_cours != nullptr) && (demo_en_cours->props.requiert_roi)) {
    ...
    cv::rectangle(Ia, rdi0, rdi1, cv::Scalar(0, 255, 0), 3);
             ...
             (demo_en_cours->props.requiert_masque)) {
...
void OCVDemo::update_Ia() { mosaique.update_image(0, Ia); }

OCVDemo *OCVDemo::get_instance() { return instance; }

OCVDemo::OCVDemo(utils::CmdeLine &cmdeline,
                 const std::string &prefixe_modele_) {
  ...
  lock = false;
  rp = nullptr;
  fs_racine = new utils::model::FileSchema(
      utils::get_fixed_data_path() + PATH_SEP + prefixe_modele + "-schema.xml");
  utils::mmi::NodeViewConfiguration vconfig;

  chemin_fichier_config = utils::get_current_user_path() + PATH_SEP + "cfg.xml";
  if (!files::file_exists(chemin_fichier_config)) {
    modele_global = utils::model::Node::create_ram_node(
        fs_racine->get_schema("global-schema"));
    modele_global.save(chemin_fichier_config);
    this->on_menu_entree();
  } else {
    modele_global = utils::model::Node::create_ram_node(
        fs_racine->get_schema("global-schema"), chemin_fichier_config);
  ...
  if (utils::files::file_exists(lockfile)) {
    if (utils::mmi::dialogs::check_dialog(langue.get_item("check-lock-1"),
                                          langue.get_item("check-lock-2"),
                                          langue.get_item("check-lock-3"))) {
      auto s = utils::mmi::dialogs::enregistrer_fichier(
          langue.get_item("save-log-title"), ".txt", "Log file");
      if (s.size() > 0) {
        if (utils::files::get_extension(s).size() == 0)
          s += ".txt";
        utils::files::copy_file(s, utils::get_current_user_path() + PATH_SEP +
                                       "ocvdemo-log.txt.old");
      }
    }
  } else
  ...
  b_entree.signal_clicked().connect(
  ...
  tdm = utils::model::Node::create_ram_node(
      schema,
  ...
  vue_arbre.utils::CProvider<utils::mmi::SelectionChangeEvent>::add_listener(
      this);
  ...
  if (cmdeline.has_option("-s")) {
    ...
    utils::files::save_txt_file(
        "../../../site/contenu/opencv/ocvdemo/table.html", s);
    s = this->export_html(Localized::Language::LANG_EN);
    utils::files::save_txt_file(
        "../../../site/contenu/opencv/ocvdemo/table-en.html", s);
    s = this->export_html(Localized::Language::LANG_DE);
    utils::files::save_txt_file(
        "../../../site/contenu/opencv/ocvdemo/table-de.html", s);
    s = this->export_html(Localized::Language::LANG_RU);
    utils::files::save_txt_file(
        "../../../site/contenu/opencv/ocvdemo/table-ru.html", s);
  }

  barre_outil_dessin.b_raz.signal_clicked().connect(
      sigc::mem_fun(*this, &OCVDemo::on_b_masque_raz));
  barre_outil_dessin.b_gomme.signal_clicked().connect(
      sigc::mem_fun(*this, &OCVDemo::on_b_masque_gomme));
  barre_outil_dessin.b_remplissage.signal_clicked().connect(
      sigc::mem_fun(*this, &OCVDemo::on_b_masque_remplissage));

  maj_langue();

  std::vector<Gtk::TargetEntry> listTargets;
  listTargets.push_back(Gtk::TargetEntry("text/uri-list"));
  wnd.drag_dest_set(listTargets,
                    Gtk::DEST_DEFAULT_MOTION | Gtk::DEST_DEFAULT_DROP,
                    Gdk::ACTION_COPY | Gdk::ACTION_MOVE);
  wnd.signal_drag_data_received().connect(
      sigc::mem_fun(*this, &OCVDemo::on_dropped_file));

  maj_bts();

  this->img_selecteur.CProvider<ImageSelecteurRefresh>::add_listener(this);
  wnd.signal_delete_event().connect(
      sigc::mem_fun(*this, &OCVDemo::on_delete_event));
  ...
  // Moved "-c" from above to here so it runs after thread_calcul has been
  // started.
...
void OCVDemo::demarre_interface() { Gtk::Main::run(wnd); }

bool OCVDemo::has_output() { return sortie_en_cours; }
